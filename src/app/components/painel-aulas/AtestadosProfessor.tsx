                  ...
                  <div className="flex flex-col gap-2">
                    ...
                    {atestado.arquivoPath && (
                      <button
                        type="button"
                        onClick={() =>
                          window.open(`/api/uploads?path=${encodeURIComponent(atestado.arquivoPath.replace('/uploads/', ''))}`, '_blank')
                        }
      ...
      <div className="bg-white rounded-lg max-w-4xl w-full max-h-[90vh] overflow-y-auto">
        <div className="p-6">
          <div className="flex justify-between items-center mb-6">
            <h3 className="text-xl text-gray-900 font-semibold">Detalhes do Atestado</h3>
            <button
              onClick={onClose}
          ...
          <div className="mb-4 overflow-x-auto">
            <table className="min-w-full bg-white border border-gray-200 rounded-lg overflow-hidden">
              <thead>
                <tr>
                  <th className="px-4 py-2 bg-blue-500 text-left font-semibold text-white border-b">Campo</th>
                  <th className="px-4 py-2 bg-blue-500 text-left font-semibold text-white border-b">Valor</th>
                </tr>
              </thead>
              <tbody>
                <tr>
                  <td className="px-4 py-2 border-b text-gray-900 font-medium bg-gray-200">Nome do Aluno</td>
                  <td className="px-4 py-2 border-b text-gray-900 bg-gray-200">{atestado.aluno.nome} {atestado.aluno.sobrenome}</td>
                </tr>
                <tr>
                  <td className="px-4 py-2 border-b text-gray-900 font-medium bg-gray-200">CPF</td>
                  <td className="px-4 py-2 border-b text-gray-900 bg-gray-200">{atestado.aluno.cpf}</td>
                </tr>
                <tr>
                  <td className="px-4 py-2 border-b text-gray-900 font-medium bg-gray-200">Status</td>
                  <td className="px-4 py-2 border-b text-gray-900 bg-gray-200">{atestado.status}</td>
                </tr>
                <tr>
                  <td className="px-4 py-2 border-b text-gray-900 font-medium bg-gray-200">Motivo</td>
                  <td className="px-4 py-2 border-b text-gray-900 bg-gray-200">{atestado.motivo}</td>
                </tr>
                <tr>
                  <td className="px-4 py-2 border-b text-gray-900 font-medium bg-gray-200">Período</td>
                  <td className="px-4 py-2 border-b text-gray-900 bg-gray-200">
                    {new Date(atestado.dataInicio).toLocaleDateString('pt-BR')} a {new Date(atestado.dataFim).toLocaleDateString('pt-BR')}
                  </td>
                </tr>
                <tr>
                  <td className="px-4 py-2 border-b text-gray-900 font-medium bg-gray-200">Enviado em</td>
                  <td className="px-4 py-2 border-b text-gray-900 bg-gray-200">{new Date(atestado.dataEnvio).toLocaleDateString('pt-BR')}</td>
                </tr>
                {atestado.observacoes && (
                  <tr>
                    <td className="px-4 py-2 border-b text-gray-900 font-medium bg-gray-200">Observações do Aluno</td>
                    <td className="px-4 py-2 border-b text-gray-900 bg-gray-200">{atestado.observacoes}</td>
                  </tr>
                )}
              </tbody>
            </table>
          ...
          <div className="mb-6">
            <h4 className="font-semibold mb-3 text-gray-900">Aulas Afetadas ({atestado.aulasJustificadas.length})</h4>
                ...
                <tbody className="divide-y divide-gray-200">
                  {atestado.aulasJustificadas.map((item, index) => (
                    <tr key={index} className="hover:bg-gray-50">
                      <td className="px-4 py-2 text-gray-900 bg-gray-200">{new Date(item.aula.dataAula).toLocaleDateString('pt-BR')}</td>
                      <td className="px-4 py-2 text-gray-900 bg-gray-200">{item.aula.horario}</td>
                      <td className="px-4 py-2 text-gray-900 bg-gray-200">{item.aula.materia.nomeMateria}</td>
                      <td className="px-4 py-2 text-gray-900 bg-gray-200">{item.aula.turma.nomeTurma}</td>
                      <td className="px-4 py-2 text-gray-900 bg-gray-200">
                ...
                <button
                  ...
                  className={`px-4 py-2 rounded-lg ${
                    acaoSelecionada === 'Aprovado'
                      ...
                      : 'bg-gray-200 text-white hover:bg-white'
                  }`}
                >
                  ✅ Aprovar
                </button>
