...
export function GerenciarAulasRecorrentes({
  ...
  // Novo: força atualização dos dados externos
  ...
  useEffect(() => {
    // Só tenta buscar se o endpoint GET existir, senão ignora o erro 405
    fetch("/api/dias-nao-letivos", { method: "GET" })
      .then((res) => {
        if (res.ok) return res.text();
        // Se não for GET permitido (405), retorna array vazio e não loga erro
        if (res.status === 405) return "[]";
        // Para outros erros, ainda retorna array vazio
        return "[]";
      })
      .then((text) => {
        try {
      ...
      .catch(() => {
        // Silencia qualquer erro de rede
        setFeriados([]);
      });
  }, [feriadoMsg]);
  ...
  async function handleAgendar(e: React.FormEvent) {
    ...
    try {
      ...
      if (res.status === 201) {
        // Mostra motivos das puladas, se houver
        let msg = `Aulas criadas: ${Array.isArray(result.criadas) ? result.criadas.length : 0}. Puladas: ${Array.isArray(result.puladas) ? result.puladas.length : 0}.`;
        if (Array.isArray(result.puladas) && result.puladas.length > 0) {
          const motivos = Array.from(
            new Set(
              (result.puladas as { motivo: string }[]).map((p) => p.motivo)
            )
          ).join("; ");
          msg += ` Motivos: ${motivos}`;
        }
        setAgendamentoMsg(msg);
